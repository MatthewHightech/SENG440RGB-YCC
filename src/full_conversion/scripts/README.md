...
Run everything from `src/full_conversion/` (parent of this `scripts/` directory),
...
| Script | Purpose |
|--------|---------|
| `prepare_test_image.py` | Build a sized PPM (64×48 or 640×480) from course `.data` / `.raw` or a PNG/JPEG |
...
```bash
make native
./csc_full_conversion testimages/input.ppm testimages/output.ppm \
